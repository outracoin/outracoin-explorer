...
function get_summary(coin, exchange, cb) {
  var req_url = base_url + '/ticker/' + coin + '_' + exchange; // https://yobit.io/api/3/ticker/ltc_btc
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + '/trades/' + coin + '_' + exchange; // https://yobit.io/api/3/trades/ltc_btc
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
      return cb(error, null);
    } else {
...
function get_orders(coin, exchange, cb) {
  var req_url = base_url + '/depth/' + coin + '_' + exchange; // https://yobit.io/api/3/depth/ltc_btc
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.success == 0) {
