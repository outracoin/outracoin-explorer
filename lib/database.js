...
module.exports = {
  ...
  find_peer: function(peerobj, cb) {
    Peers.findOne({address: peerobj['address'], port: peerobj['port']}, function(err, peer) {
      if (err) {
        return cb({
          address: peerobj['address'],
          port: peerobj['port'],
          protocol: peerobj['protocol'],
          version: peerobj['version'],
          status: "error"
        });
      } else {
        if (peer) {
  ...
  drop_peers: function(address, cb) {
    //Peers.deleteMany({}, function(err) {
    Peers.deleteMany({address: address}, function(err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
